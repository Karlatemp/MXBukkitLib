 ...
 * Reserved.FileName: BasicImageReader.java@author: karlatemp@vip.qq.com: 2019/12/27 下午7:25@version: 2.0
 */
...
public class BasicImageReader implements AutoCloseable {
    private static boolean isSystemProperty(String key, String value, String def) {
        ...
        return AccessController.doPrivileged(
                (PrivilegedAction<Boolean>) () -> value.equals(System.getProperty(key, def))
        );
    }

    static private final boolean IS_64_BIT =
            isSystemProperty("sun.arch.data.model", "64", "32");
    // static private final boolean USE_JVM_MAP = false;
    ...
    protected BasicImageReader(Path path, ByteOrder byteOrder)
        ...
        AccessController.doPrivileged((PrivilegedAction<Void>) () -> {
            try {
                ...
                Reflect.ofClass(fileChannelImpl).getMethod("setUninterruptible", new Class[0]).invoke(channel);
            } catch (Throwable ex) {
    ...
    public Stream<String> getEntryNamesStream() {
    ...
    public String[] getEntryNames() {
        return getEntryNamesStream().toArray(String[]::new);
    }
