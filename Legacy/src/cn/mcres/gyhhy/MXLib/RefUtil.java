 ...
 * Reserved.FileName: RefUtil.java@author: karlatemp@vip.qq.com: 19-9-18 下午5:54@version: 2.0
 */
...
public class RefUtil {
    ...
    private static final MethodHandle defineClass, findLoadedClass;

    static {
        Looker lk = new Looker(Looker.openLookup(ClassLoader.class, ~0));
        defineClass = lk
                .findVirtual(ClassLoader.class, "defineClass", MethodType.methodType(
                        Class.class, String.class, byte[].class, int.class, int.class
                ));
        findLoadedClass = lk.findVirtual(ClassLoader.class, "findLoadedClass", MethodType.methodType(
                Class.class, String.class
        ));
    }

    public static Class<?> safeLoadClass(String cname) {
        return safeLoadClass(cname, false, null);
    }

    public static Class<?> findLoadedClass(ClassLoader cl, String className) {
        try {
            return (Class<?>) findLoadedClass.invoke(cl, className);
        } catch (Throwable ex) {
    ...
    public static Class<?> safeLoadClass(String cname, boolean initialize, ClassLoader cl) {
        try {
            ...
            return Class.forName(cname, initialize, cl);
    ...
    public static boolean classExists(String cname) {
        return classExists(cname, false, null);
    }

    public static boolean classExists(String cname, boolean initialize, ClassLoader cl) {
        return safeLoadClass(cname, initialize, cl) != null;
    }
    ...
    public static ClassLoader getExtClassLoader() {
        ...
        ClassLoader ext = null;
        while (cl != null) {
            String name = cl.getClass().getName();
            if (name.endsWith("Launcher$ExtClassLoader") && name.startsWith("sun.")) {
                ext = cl;
                break;
            }
            cl = cl.getParent();
