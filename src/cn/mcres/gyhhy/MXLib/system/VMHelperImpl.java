...
public class VMHelperImpl extends VMHelper {
    ...
    static {
        ...
        if (AgentCodeShare.setup) { // Check is setuped
            ...
            try {
                ...
                // Search JDK Tool
                {
                    ...
                    Plugin.getLoggerEX().printf("Patch java home: " + jhome);
                    jdktool = new File(jhome, "lib/tools.jar");
                    if (jdktool.isFile()) {
                        Plugin.getLoggerEX().printf("Found jdk tools.");
                        Plugin.getLoggerEX().printf("At " + jdktool);
                    } else {
                        Plugin.getLoggerEX().printf("Unfound jdk tools.");
                        jdktool = new File(Plugin.plugin.getDataFolder(), "jdktool.jar");
                        final File jdktools = jdktool;
                        if (!jdktool.isFile()) {
                            String link = ConfSave.jvm_jdk_tool;
                            Plugin.getLoggerEX().printf("\u00a7bDownload jdk tool from \u00a76" + link);
                            LongStore ls = new LongStore(System.currentTimeMillis());
                            WebHelper.http(link).response((code, connection, is) -> {
                                if (code != 200) {
                                    throw new java.lang.NullPointerException();
                                }
                                Plugin.getLoggerEX().printf("\u00a7bConnected. Start download." + (System.currentTimeMillis() - ls.v()) + "ms");
                                    ...
                                    try (FileOutputStream fos = new FileOutputStream(jdktools)) {
                                        ...
                                        while ((leng = is.read(buffer)) != -1) {
                                            ...
                                            if (System.currentTimeMillis() - ls.v() > 200) {
                                                ...
                                                Plugin.getLoggerEX().printf("Downloaded " + ap);
                                            }
                                            ls.v(System.currentTimeMillis());
...
//                    ucl.loadClass("com.sun.tools.attach.VirtualMachine");
        ...
        try {
            ClassLoader.getSystemClassLoader().loadClass(AgentStartup_c); // Check was core code loaded?
