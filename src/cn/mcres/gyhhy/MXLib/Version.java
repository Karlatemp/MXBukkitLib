...
import cn.mcres.gyhhy.MXLib.io.StringReader;
...
public class Version implements Comparable<Version> {
    ...
    public static void checkThrow(String ver) {
        if (!check(ver)) {
            throw new java.lang.UnsupportedOperationException(ver + " is not Semantic Version");
        }
    }

    public static boolean check(String ver) {
        StringReader rd = new StringReader(ver);
        if (!isV(rd.read(), 0)) {
            return false;
        }
        // read x,y,z(and more?)
    ...
    private static int[] parse(String buff) {
        StringReader rd = new StringReader(buff);
        int lg = 1;
        while (true) {
            int i = rd.read();
            if (i == -1) {
